...
class PCA(object):
    ...
    def fit(self, X, method='svd'):
        ...
        self.mean = np.mean(X) * np.ones(D)
        X_centered = X - self.mean
        if(method == "svd"):
            self.W_pca, _ = self._svd(X)
            print("size:",np.shape(self.W_pca),D)
        elif(method == "eigen"):
            self.W_pca, _ = self._eigen_decomp(X)
            print("size:",np.shape(self.W_pca))
        else:
            self.W_pca = np.zeros((D, D))
        # END YOUR CODE
        ...
        # Each column of `self.W_pca` should have norm 1 (each one is an
        # eigenvector)
        '''
        for i in range(D):
    ...
    def _eigen_decomp(self, X):
        ...
        # 3. Sort both of them in decreasing order (ex: 1.0 > 0.5 > 0.0 > -0.2
        # > -1.2)
        meanc = np.mean(X, axis=0)  # D
        #print("mean:", meanc)
        # print("ND:",N,D)
        conva = np.zeros((D, D))
        for i in range(D):
            for j in range(D):
                conva[i, j] = np.sum((X[:, i] - meanc[i])
                                     * (X[:, j] - meanc[j])) / (N - 1)
        #print("covar:", conva)
        #print("conv shape",np.shape(conva))
        vl, vc = scipy.linalg.eig(conva)
        vl = abs(vl)
        vc = abs(vc)
        #print("vl,vc:", vl, vc)
        #print("vl,vc,shape", np.shape(vl), np.shape(vc))
        idx = np.flipud(np.argsort(vl))
        e_vals = np.array([vl[idx[i]] for i in range(D)])
        e_vecs = np.array([vc[:, idx[i]] for i in range(D)])
        #print("eigvalues:", e_vals)
        # END YOUR CODE
    ...
    def _svd(self, X):
        ...
        # Make sure to return vecs as the matrix of vectors where each column
        # is a singular vector
        M, vals, vecs = scipy.linalg.svd(X)
        #print("singvalues:", vals)
        # END YOUR CODE
    ...
    def transform(self, X, n_components):
        ...
        # 2. then project onto a subspace of dimension `n_components` using
        # `self.W_pca`
    ...
    def reconstruct(self, X_proj):
        ...
        X=np.dot(X_proj,self.W_pca[:n_components,:])+self.mean
        # END YOUR CODE
...
class LDA(object):
    ...
    def fit(self, X, y):
        ...
        vl, vc = scipy.linalg.eig(np.dot(np.linalg.inv(scatter_within),scatter_between))
        vl = abs(vl)
        vc = abs(vc)
        idx = np.flipud(np.argsort(vl))
        e_vecs = np.array([vc[:, idx[i]] for i in range(D)])
        ...
        # Each column of `self.W_lda` should have norm 1 (each one is an
        # eigenvector)
        for i in range(D):
            print("check:",np.linalg.norm(self.W_lda[:, i])-1.0)
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            # YOUR CODE HERE
            # Get the covariance matrix for class i, and add it to
            # scatter_within
            X_i=X[np.transpose(np.argwhere(y==i))[0]]
            meanc = np.mean(X_i, axis=0)  # D
            S_i = np.dot(np.transpose(X_i-meanc),X_i-meanc)#/(np.shape(X_i)[0]-1)
            scatter_within+=S_i
            # END YOUR CODE
    ...
    def _between_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            # YOUR CODE HERE
            X_i=X[np.transpose(np.argwhere(y==i))[0]]
            mu_i = np.mean(X_i, axis=0)  # D
            S_i = np.dot(np.transpose(mu_i-mu),mu_i-mu)*np.shape(X_i)[0]
            scatter_between+=S_i
            #  END YOUR CODE
    ...
    def transform(self, X, n_components):
        ...
        X_proj = None
        # YOUR CODE HERE
        # project onto a subspace of dimension `n_components` using
        # `self.W_lda`
        X_proj=np.dot(X,self.W_lda[:,:n_components])
        print(np.shape(X),np.shape(self.W_lda),np.shape(self.W_lda[:,:n_components]),np.shape(X_proj))
        # END YOUR CODE
