        ...
        for epoch in range(self.config.n_epochs):
            ...
            for batch in minibatches(train_examples, self.config.batch_size):
                ...
                if len(batch)==3:
                    ...
                    self.train_on_batch(sess, inputs_batch=features, labels_batch=labels, mask_batch=masks)
                else:
                    raise ValueError("train examples has %d element for each sentence! But the supporteds are 2 and 3"%len(batch))
            ### END YOUR CODE
