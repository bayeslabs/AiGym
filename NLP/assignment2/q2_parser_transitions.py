class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            raise(KeyError("Transition str key {} is not valide".format(transition)))
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        pars=unfinished_parses[:batch_size]
        transitions=model.predict(pars)
        tobe_deleted=[]
        for i,p in enumerate(pars):
            p.parse_step(transitions[i])
            if len(p.buffer)==0 and len(p.stack)==1:
                tobe_deleted.append(i)
        for i in reversed(tobe_deleted):
