...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self):
        ...
        preds=tf.nn.sigmoid(output[0:,-1,:])
        ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        loss=tf.reduce_mean(tf.nn.l2_loss(preds-tf.cast(y,tf.float32)))
        ### END YOUR CODE
    ...
    def add_training_op(self, loss):
        ...
        # is True.
        # - Remember to set self.grad_norm
        self.grad_norm, variables=zip(*optimizer.compute_gradients(loss))
        if self.config.clip_gradients:
            self.grad_norm,_=tf.clip_by_global_norm(self.grad_norm, self.config.max_grad_norm)
        train_op=optimizer.apply_gradients(zip(self.grad_norm,variables))
