...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE
        self.input_placeholder=tf.placeholder(dtype=tf.int32, shape=(None, Config.n_features), name='input_placeholder')
        self.labels_placeholder=tf.placeholder(dtype=tf.float32, shape=(None, Config.n_classes), name='labels_placeholder')
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=0):
        ...
        feed_dict[self.dropout_placeholder]=1-dropout
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embedding_weight=tf.Variable(self.pretrained_embeddings)
        embeddings=tf.nn.embedding_lookup(embedding_weight, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        W1 = tf.Variable(xavier_weight_init()(shape=(Config.n_features*Config.embed_size,self.config.hidden_size)), name='W1')
        W2 = tf.Variable(xavier_weight_init()(shape=(Config.hidden_size,self.config.n_classes)), name='W2')
        ...
        z = tf.add(tf.matmul(h,W2),b2, name='z')
        pred = tf.nn.dropout(z, self.dropout_placeholder, name='pred')
        ### END YOUR CODE
        return pred
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=pred, labels=self.labels_placeholder, name='loss'))
        ### END YOUR CODE
        return loss
    ...
    def fit(self, sess, saver, parser, train_examples, dev_set):
        ...
        for epoch in range(self.config.n_epochs):
            ...
            if dev_UAS > best_dev_UAS:
                ...
                if saver:
                    path=os.path.join(os.path.dirname(__file__),'data/weights/parser.weights') 
                    print("New best dev UAS! Saving model in %s"%path)
...
def main(debug=False):
    ...
    path=os.path.join(os.path.dirname(__file__),'data/weights') 
    ...
    with tf.Session(graph=graph) as session:
        ...
        if not debug:
            ...
            saver.restore(session, os.path.join(path,'parser.weights'))
            print("Final evaluation on test set", end=' ')
            UAS, dependencies = parser.parse(test_set)
            print("- test UAS: {:.2f}".format(UAS * 100.0))
            print("Writing predictions")
            with open(os.path.join(os.path.dirname(__file__),'q2_test.predicted.pkl'), 'wb') as f:
