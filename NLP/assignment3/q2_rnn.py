...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        tmp=max(0,max_length-len(sentence))
        mask=len(sentence)*[True]+tmp*[False]
        sentence=sentence[:max_length]+[zero_vector]*tmp
        labels=labels[:max_length]+[zero_label]*tmp
        mask[tmp:]
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings=tf.nn.embedding_lookup(embedding_tensor, self.input_placeholder) # [None,max_len,n_feathres,embed_size]
    ...
    def add_prediction_op(self):
        ...
        state = tf.zeros(dtype=tf.float32, shape=(1,Config.hidden_size), name='state')
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            ...
            for time_step in range(self.max_length):
                ...
                pred = tf.add(tf.matmul(out_drop,U),b2,name='pred')
        ...
        preds=tf.reshape(tf.stack(preds,axis=0),shape=(-1,self.max_length, self.config.n_classes))
        ### END YOUR CODE
