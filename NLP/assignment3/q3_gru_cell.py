...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_z=tf.get_variable(name="W_z", shape=(self.input_size, self.output_size), initializer=tf.contrib.layers.xavier_initializer())
            U_z=tf.get_variable(name="U_z", shape=(self.output_size, self.output_size), initializer=tf.contrib.layers.xavier_initializer())
            b_z=tf.get_variable(name='b_z', shape=(self.output_size))
            W_r=tf.get_variable(name="W_r", shape=(self.input_size, self.output_size), initializer=tf.contrib.layers.xavier_initializer())
            U_r=tf.get_variable(name="U_r", shape=(self.output_size, self.output_size), initializer=tf.contrib.layers.xavier_initializer())
            b_r=tf.get_variable(name='b_r', shape=(self.output_size))
            W_o=tf.get_variable(name="W_o", shape=(self.input_size, self.output_size), initializer=tf.contrib.layers.xavier_initializer())
            U_o=tf.get_variable(name="U_o", shape=(self.output_size, self.output_size), initializer=tf.contrib.layers.xavier_initializer())
            b_o=tf.get_variable(name='b_o', shape=(self.output_size))
            z_t=tf.nn.sigmoid(tf.add(tf.matmul(inputs,W_z)+tf.matmul(state,U_z),b_z),name='z_t')
            r_t=tf.nn.sigmoid(tf.add(tf.matmul(inputs,W_r)+tf.matmul(state,U_r),b_r),name='r_t')
            o_t=tf.nn.tanh(tf.add(tf.matmul(inputs,W_o)+r_t*tf.matmul(state,U_o),b_o),name='o_t')
