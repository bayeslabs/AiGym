...
def compute_distances(X1, X2):
    ...
    vec=np.reshape(np.tile(X1,N),(M,N,X1.shape[1]))-np.reshape(np.tile(np.reshape(X2,-1),M),(M,N,X1.shape[1]))
    for i in range(M):
        for j in range(N):
            dists[i,j]=np.linalg.norm(vec[i,j])
    # END YOUR CODE
...
def predict_labels(dists, y_train, k=1):
    ...
    for i in range(num_test):
        ...
        # YOUR CODE HERE
        s=y_train[ np.argsort(dists[i])[0:k] ]#classes in neighbor
        count = np.bincount(s)
...
def split_folds(X_train, y_train, num_folds):
    ...
    for i in range(num_folds):
        ...
        y = np.array_split(y_train, num_folds, 0)
        X_vals[i]=x[i]
        y_vals[i]=y[i]
        del x[i]
        del y[i]
        for j in range(num_folds-1):
            X_trains[i,j*validation_size:(j+1)*validation_size]=x[j]
            y_trains[i,j*validation_size:(j+1)*validation_size]=y[j]
    # END YOUR CODE
