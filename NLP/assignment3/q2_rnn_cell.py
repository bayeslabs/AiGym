...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_h = tf.get_variable(name='W_h', shape=(self.state_size,self.output_size), initializer=tf.contrib.layers.xavier_initializer())
            z_t=tf.add(tf.matmul(inputs,W_x)+tf.matmul(state,W_h),b,name='z_t')
            new_state=tf.nn.sigmoid(z_t, name='new_state')
            ### END YOUR CODE ###
