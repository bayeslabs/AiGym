...
class Config:
    ...
    def __init__(self, output_path=None):
        if output_path:
            ...
            self.output_path = os.path.join(os.path.dirname(__file__),"results/window/{:%Y%m%d_%H%M%S}/".format(datetime.now()))
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        tmp_sentence=Config.window_size*[start]+sentence+Config.window_size*[end]
        for i in range(len(sentence)):
            feats=tmp_sentence[i:i+Config.n_window_features//Config.n_word_features]
            label=labels[i]
            windowed_data.append(([p for f in feats for p in f],label))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder=tf.placeholder(dtype=tf.int32,shape=(None,Config.n_window_features), name="input_placeholder")
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embedding_tensor=tf.Variable(self.pretrained_embeddings, name="embeddings")
    ...
    def add_prediction_op(self):
        ...
        w1=tf.get_variable("w1", shape=(Config.n_window_features*Config.embed_size,Config.hidden_size), initializer=tf.contrib.layers.xavier_initializer())
        b1=tf.Variable(tf.zeros(Config.hidden_size),name='b1')
        w2=tf.get_variable("w2", shape=(Config.hidden_size,Config.n_classes), initializer=tf.contrib.layers.xavier_initializer())
        b2=tf.Variable(tf.zeros(Config.n_classes), name='b2')
        h = tf.nn.relu(tf.matmul(x,w1)+b1,name="h")
        h_drop = tf.nn.dropout(h,dropout_rate)
        pred=tf.add(tf.matmul(h_drop,w2),b2,name="pred")
        ### END YOUR CODE
        return pred
